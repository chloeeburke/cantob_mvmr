...
##################
setwd(" ")
write.csv(cud_dat_mr_clumped,"cud_dat_mvmr.csv", row.names = FALSE)

###################
## Reading files ##
###################
#setwd(" ")
#cud_dat_mr_clumped <- read.csv("cud_dat_mvmr.csv", header = T)
...
setwd(" ")
si_cud_exp <- read.csv("SI_CUD_iPSYCH.csv", header = T)
...
#Removing the following SNPs (n = 13) for being palindromic with intermediate allele frequencies:
...
###########################################
outcome_dat_sicud$beta.outcome <- as.numeric(
  as.character(
    outcome_dat_sicud$beta.outcome
  )
)
outcome_dat_sicud["beta.outcome"] <- (outcome_dat_sicud$beta.outcome/(0.22652*((1-0.22652))))
outcome_dat_sicud["se.outcome"] <- (outcome_dat_sicud$se.outcome/(0.22652*((1-0.22652))))
...
mvdat_sicud1 <- read.csv("mvdat_sicud_mdd.csv", header = T)
...
cov_adj_mvmr_sicud <- qhet_mvmr(df_mvmr_sicud, cov, CI = TRUE, iterations = 1000)
