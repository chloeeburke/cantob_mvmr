...
# The script conducts multivariable MR exploring the effects ofsmoking initiation and cannabis initation on lifetime depression.
...
##################
setwd(" ")
write.csv(ci_dat_mr_clumped,"ci_dat_mvmr", row.names = FALSE)

###################
## Reading files ##
###################
#setwd(" ")
#ci_dat_mr_clumped <- read.csv("ci_dat_mvmr", header = T)
...
#Manually edited SNP lists swapping: rs2359180 in 'SI_SNPlist' for rs12607929, and rs9773390 in 'CI_SNPlist' for rs73691962
...
##################
setwd(" ")
write.csv(si_dat_mr_proxy_clumped,"si_dat_mvmr_proxy", row.names = FALSE)
write.csv(ci_dat_mr_proxy_clumped,"ci_dat_mvmr_proxy", row.names = FALSE)

###################
## Reading files ##
###################
#setwd(" ")
#si_dat_mr_proxy_clumped <- read.csv("si_dat_mvmr_proxy", header = T)
#ci_dat_mr_clumped <- read.csv("ci_dat_mvmr_proxy", header = T)
...
outcome_dat_sici$beta.outcome<-log(outcome_dat_sici$beta.outcome)
...
write.csv(
  mvdat_sici1,
  "mvdat_sici_howard.csv",
  row.names = FALSE
)


mvdat_sici1 <- read.csv("mvdat_sici_howard.csv", header = T)
